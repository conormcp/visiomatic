...
L.Projection.WCS = L.Class.extend({
  ...
  project: function (latlng) {
    var phiTheta = this._raDecToPhiTheta(
      this.celsysflag ? this.eqToCelsys(latlng) : latlng
    );
  ...
  _cpole: function () {
    var deg = Math.PI / 180.0,
      ...
      ddeltap =
        Math.acos(sd0 / Math.sqrt(1.0 - ct0 * ct0 * sdphip * sdphip)) / deg,
    ...
    if (deltap1 > 90.0) {
      ...
      deltap =
        Math.abs(deltap1 - projparam.natpole.lat) <
        Math.abs(deltap2 - projparam.natpole.lat)
          ? deltap1
          : deltap2;
    }
    var alphap =
      Math.abs(projparam.crval.lat) === 90.0
        ? projparam.crval.lng
        : deltap === 90.0
        ? projparam.crval.lng +
          projparam.natpole.lng -
          projparam.natrval.lng -
          180.0
        : deltap === -90.0
        ? projparam.crval.lng - projparam.natpole.lng + projparam.natrval.lng
        : projparam.crval.lng -
          Math.atan2(
            (sdphip * ct0) / cd0,
            (st0 - Math.sin(deltap * deg) * sd0) /
              (Math.cos(deltap * deg) * cd0)
          ) /
            deg;
  ...
  _phiThetaToRADec: function (phiTheta) {
    ...
    return L.latLng(
      ...
      projparam.cpole.lng +
        Math.atan2(-ct * Math.sin(dphi), st * cdp - ct * sdp * cdphi) * rad
    );
  },
  ...
  _raDecToPhiTheta: function (raDec) {
    var projparam = this.projparam,
      ...
      phitheta = L.latLng(
        Math.asin(asinarg > 1.0 ? 1.0 : asinarg < -1.0 ? -1.0 : asinarg) * rad,
        projparam.natpole.lng +
          Math.atan2(-cd * sda, sd * cdp - cd * sdp * cda) * rad
      );
...
L.Projection.WCS.cylindrical = L.Projection.WCS.extend({
  _paramInit: function (projparam) {
    ...
    if (projparam.lambda === 0.0) {
      projparam.lambda = 1.0;
    }
...
L.Projection.WCS.CEA = L.Projection.WCS.cylindrical.extend({
  ...
  _redToPhiR: function (red) {
    ...
    return L.latLng(
      slat > -1.0 ? (slat < 1.0 ? Math.asin(slat) / deg : 90.0) : -90.0,
      red.x
    );
  },
...
L.Projection.WCS.conical = L.Projection.WCS.extend({
  ...
  _redToPhiR: function (red) {
    ...
    return L.latLng(
      rTheta,
      Math.atan2(red.x / rTheta, dy / rTheta) / projparam.c / deg
    );
  },
  ...
  _phiRToRed: function (phiR) {
    ...
    return L.point(
      phiR.lat * Math.sin(p),
      -phiR.lat * Math.cos(p) + this.projparam.y0
    );
  },
});
...
L.Projection.WCS.COE = L.Projection.WCS.conical.extend({
  _paramInit: function (projparam) {
    ...
    projparam.y0 =
      ((2.0 / projparam.gamma) *
        Math.sqrt(
          projparam.s1s2p1 - projparam.gamma * Math.sin(projparam.thetaA * deg)
        )) /
      deg;
  ...
  _rToTheta: function (r) {
    var deg = Math.PI / 180.0,
      ...
      sinarg =
        this.projparam.s1s2p1 / gamma - (gamma * r * r * deg * deg) / 4.0;
    if (sinarg < -1.0) {
      sinarg = -1.0;
    } else if (sinarg > 1.0) {
      sinarg = 1.0;
  ...
  _thetaToR: function (theta) {
    ...
    return (
      ((2.0 / gamma) *
        Math.sqrt(this.projparam.s1s2p1 - gamma * Math.sin(theta * deg))) /
      deg
    );
  },
});

...
L.Control.Layers.IIP = L.Control.Layers.extend({
  ...
  _initLayout: function () {
    ...
    if (this.options.fileMenu) {
      var addbutton = (this._addButton = L.DomUtil.create(
        "input",
        className + "-add",
        form
      ));
  ...
  _addItem: function (obj) {
    ...
    var trashbutton = L.DomUtil.create(
      "input",
  ...
  _openFileMenu: function () {
    var _this = this,
      fileMenu = L.DomUtil.create(
        "div",
        "leaflet-control-filemenu",
        this._map._controlContainer
      );
    fileMenu.title = "Open file";
    this._addButton.disabled = true;
    L.DomEvent.disableClickPropagation(fileMenu).disableScrollPropagation(
      fileMenu
    );
    ...
    var fileTree = L.DomUtil.create(
      "div",
      "leaflet-control-filetree",
      fileMenu
    );
    ...
    $(document).ready(function () {
      $("#leaflet-filetree").fileTree(
        ...
        function (fitsname) {
          ...
          $.post(
            _this.options.fileProcessScript,
            {
              fitsname: fitsname,
            },
            function (ptifname) {

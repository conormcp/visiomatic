...
L.Control.Scale.WCS = L.Control.Scale.extend({
  ...
  _update: function () {
    ...
    if (options.custom && crs.options && crs.options.nzoom) {
      var customScale =
        Math.pow(2.0, crs.options.nzoom - 1 - map.getZoom()) *
        options.customScale;
  ...
  _jacobian: function (latlng) {
    ...
    return (
      0.01 *
      Math.abs(
        (latlngdx.lng - latlng.lng) * (latlngdy.lat - latlng.lat) -
          (latlngdy.lng - latlng.lng) * (latlngdx.lat - latlng.lat)
      )
    );
  },

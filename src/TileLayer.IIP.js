...
L.TileLayer.IIP = L.TileLayer.extend({
  ...
  _parseMetadata: function (layer, httpRequest) {
    if (httpRequest.readyState === 4) {
      if (httpRequest.status === 200) {
        ...
        if (!matches) {
          alert(
            "Error: Unexpected response from IIP server " +
        ...
        layer.wcs = options.crs
          ? options.crs
          : new L.CRS.WCS(response, {
  ...
  _addToMap: function (map) {
    ...
    if (
      ...
      var latlng =
        typeof this.options.center === "string"
            ...
            if (httpRequest.readyState === 4) {
              if (httpRequest.status === 200) {
                ...
                if (latlng) {
                  ...
                  map.setView(newcrs.projparam.crval, zoom, {
                    reset: true,
                    animate: false,
                  });
                ...
                alert(
                  "There was a problem with the request to the Sesame service at CDS"
                );
              }
            }
          },
  ...
  _getTileSizeFac: function () {
    ...
    return zoomN && zoom > zoomN
      ? Math.round(map.getZoomScale(zoom) / map.getZoomScale(zoomN))
      : 1;
  },
  ...
  _isValidTile: function (coords) {
    ...
    if (
      wcoords.x < 0 ||
      wcoords.x >= this.iipGridSize[z].x ||
      wcoords.y < 0 ||
      wcoords.y >= this.iipGridSize[z].y
    ) {
      return false;
    }
  ...
  getTileUrl: function (coords) {
    ...
    for (var c = 0; c < this.iipNChannel; c++) {
      if (
        this.iipMinValue[c] !== this.iipdefault.minValue[c] ||
        ...
        str +=
          ...
          (c + 1).toString() +
          ":" +
          this.iipMinValue[c].toString() +
          "," +
          this.iipMaxValue[c].toString();
      }
    }
  ...
  _initTile: function (tile) {
    ...
    if (z > this.iipMaxZoom) {
      z = this.iipMaxZoom;
    }
    var sizeX =
        coords.x + 1 === this.iipGridSize[z].x
          ? this.iipImageSize[z].x % this.iipTileSize.x
          : this.iipTileSize.x,
      sizeY =
        coords.y + 1 === this.iipGridSize[z].y
          ? this.iipImageSize[z].y % this.iipTileSize.y
          : this.iipTileSize.y;

...
L.Control.IIP.Overlay = L.Control.IIP.extend({
  ...
  _initDialog: function () {
    ...
    // CDS catalog overlay
    elem = this._addDialogLine(
      '<a id="logo-cds" ' + 'href="http://cds.u-strasbg.fr">&nbsp;</a> catalog:'
    );
    ...
    var profbutton1 = L.DomUtil.create(
      "input",
      className + "-profile-start",
      elem
    );
    ...
    var profbutton2 = L.DomUtil.create(
      "input",
  ...
  _getCatalog: function (catalog) {
    var _this = this,
      ...
      dlng =
        Math.max(c[0].lng, c[1].lng, c[2].lng, c[3].lng) -
        Math.min(c[0].lng, c[1].lng, c[2].lng, c[3].lng),
      dlat =
        Math.max(c[0].lat, c[1].lat, c[2].lat, c[3].lat) -
      ...
      if (httpRequest.status === 200) {
            ...
            pointToLayer: function (feature, latlng) {
              return L.circleMarker(latlng, {
                radius: feature.properties.mags[0]
                  ? 8 + catalog.maglim - feature.properties.mags[0]
                  : 8,
              });
            },
        ...
        if (layercontrol) {
          ...
          layercontrol.addOverlay(
            geocatalog,
            catalog.name + " (" + geo.features.length.toString() + " entries)"
          );
          if (layercontrol.options.collapsed) {
        ...
        alert(
          "There was a problem with the request to " + catalog.service + "."
        );
      }
    }
  },
  ...
  _profileEnd: function (e) {
    ...
    line
      .bindPopup(popdiv, { minWidth: 16, maxWidth: 1024, closeOnClick: false })
      .openPopup();
  ...
  _getMeasurementString: function () {
    ...
    distance =
      ...
      L.IIPUtils.distance(currentLatLng, previousLatLng);

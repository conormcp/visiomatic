...
L.Control.WCS = L.Control.extend({
  ...
  onAdd: function (map) {
    ...
    var _this = this,
      ...
      dialog = (this._wcsdialog = L.DomUtil.create(
        "div",
        className + "-dialog"
      )),
    ...
    var input = (this._wcsinput = L.DomUtil.create(
      "input",
      className + "-input",
      dialog
    ));
    ...
    var clipboardbutton = L.DomUtil.create(
      "div",
      className + "-clipboard",
      dialog
    );
  ...
  _onDrag: function (e) {
    ...
    if (wcs.pixelFlag) {
      this._wcsinput.value =
        latlng.lng.toFixed(0) + " , " + latlng.lat.toFixed(0);
    } else {
      ...
      switch (coord.units) {
        ...
        case "deg":
          this._wcsinput.value =
            latlng.lng.toFixed(5) + " , " + latlng.lat.toFixed(5);
          break;
        default:
          this._wcsinput.value =
            latlng.lng.toFixed(1) + " , " + latlng.lat.toFixed(1);
          break;
      }
    }
  },
